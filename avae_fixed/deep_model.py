...
class DeepModel():
    ...
    def build_model(self, use_mean_lstm = True, old_version = True,
        ...
        #self.l_ax = DropoutLayer( batch_norm( DenseLayer(self.l_c, num_units=self.enc_num_units,
                                        #W = W_init_act, b= b_init,
                                        #nonlinearity= self.transf,name = 'ax'),
                                        #alpha = self.bnalpha,name = 'ax'), p = self.drop_out, name = 'ax')
    ...
    def cost_label(self, incomings, dev_stage = False, return_mode = 'sum'):
        ...
        loss_recons = (loss_recons.reshape((-1,mask.shape[1])) * mask).sum(-1) - 500.0 # matrix batch_size*sent_length --> vector batch_size
        # kl loss
