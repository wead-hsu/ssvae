...
class DeepModel():
    ...
    def build_model(self, use_mean_lstm = True, old_version = True,
        ...
        self.l_classifier_ebd = EmbeddingLayer(self.l_sents_in,self.word_dict_size, self.word_ebd_dims, W = word_ebd_init,
                                         name = 'Classifier EbdLayer')
        self.l_classifier_ebd_drop = DropoutLayer(self.l_classifier_ebd, p=self.drop_out,
                                       name='Classifier EbdDropoutLayer')  # no params; input: batch_size*sent_length*word_ebd_dims
        ####################encoder lstm layers######################################
        ...
        if use_mean_lstm:
            ...
            self.l_c0 = MeanLstmLayer(self.l_classifier_ebd, num_units = self.enc_num_units, mask_input = self.l_mask_in,
                                            grad_clipping = self.grad_clipping, name='ClassLSTMLayer')
            self.l_c = DropoutLayer(self.l_c0, p = self.drop_out, name='ClassLSTMLayer')

        else:
            self.l_c0 = LSTMLayer(self.l_classifier_ebd, num_units = self.enc_num_units, mask_input = self.l_mask_in,
        ...
        ######################## hiddens for LSTM #################
        self.l_u_in = InputLayer((None, self.dima))
        self.l_yz_in = InputLayer((None, self.dimy+self.dimz))
        self.l_uyz = ConcatLayer([self.l_yz_in, self.l_u_in], axis = 1, name='Concat_uyz')
    ...
    def get_params(self, only_trainable = False):
        ...
        if only_trainable:
            return get_all_params([self.l_y,self.l_z,self.l_u,self.l_hid,self.l_lm,self.l_recons_x],
                                             trainable = True)
    ...
    def _gen_lm_init(self, incomings, dev_stage=False):
        ...
        a, y, z = incomings
        yz, u_mu, u_var, u = get_output([self.l_yz, self.l_u_mu, self.l_u_var, self.l_u],
                                        {self.l_z_in: z, self.l_label_in: y},
                                        deterministic=dev_stage)
        if dev_stage:
            hid = get_output(self.l_hid, {self.l_yz_in: yz, self.l_u_in: u},
                                        deterministic=dev_stage)
        else:
            hid = get_output(self.l_hid, {self.l_yz_in: yz, self.l_u_in: a},
                             deterministic=dev_stage)
        return u_mu, u_var, u, hid
    ...
    def cost_label(self, incomings, dev_stage = False, return_mode = 'sum'):
        ...
        u_mu, u_var, u, hid = self._gen_lm_init([a,y,z], dev_stage=dev_stage)
    ...
    def load_pretrain(self, pretrain_path):
        ...
        with open(pretrain_path, 'rb') as f:
            ...
            wemb = self.l_classifier_ebd.get_params()
