...
class DeepModel():
    def __init__(self,
                ...
                grad_clipping = 5.0, max_norm = 20.0,
                cost_beta = 1.0, lr = 0.0008, beta1 = 0.9, beta2 = 0.999
    ...
    def build_model(self, use_mean_lstm = True, old_version = True,
        ...
        self.l_ebd =  EmbeddingLayer(self.l_sents_in,self.word_dict_size, self.word_ebd_dims, W = word_ebd_init,
                                         name = 'EbdLayer' )
        ####################encoder lstm layers######################################
        self.l_x = DropoutLayer( LSTMLayer(self.l_ebd, num_units = self.enc_num_units, mask_input = self.l_mask_in,
        ...
        if use_mean_lstm:
            ...
            self.l_c0 = MeanLstmLayer(self.l_ebd, num_units = self.enc_num_units, mask_input = self.l_mask_in,
                                            grad_clipping = self.grad_clipping, name='ClassLSTMLayer')
            self.l_c = DropoutLayer(self.l_c0, p = self.drop_out, name='ClassLSTMLayer')

        else:
            self.l_c0 = LSTMLayer(self.l_ebd, num_units = self.enc_num_units, mask_input = self.l_mask_in,
                                            grad_clipping = self.grad_clipping, only_return_final = True, name='ClassLSTMLayer')
            self.l_c = DropoutLayer(self.l_c0, p = self.drop_out, name='ClassLSTMLayer')
        #----------------- auxiliary q(a|x) ###########################################
        if old_version:
            self.l_x_to_a = batch_norm( DenseLayer(self.l_x, num_units= self.dima, W = W_init_act, b = b_init,
                                                nonlinearity= self.transf, name = 'x_to_a_old'),
                                                alpha = self.bnalpha, name = 'x_to_a_old')
        else:
            print 'Using new version of model!!!!!!!!!!'
            self.l_mean_pooling =DropoutLayer(MeanMaskLayer(self.l_ebd, self.l_mask_in, name='mean_pooling'),
                                              p = self.drop_out, name = 'mean_pooling')
            self.l_x_to_a = batch_norm( DenseLayer(self.l_mean_pooling, num_units= self.dima,
                                                W = W_init_act, b = b_init, nonlinearity= self.transf,
                                                name = 'x_to_a_new'), alpha = self.bnalpha, name = 'x_to_a_new')

        self.l_a_mu = batch_norm(DenseLayer(self.l_x_to_a, self.dima, W = W_init, b = b_init, nonlinearity=None,
                                             name = 'a_mu'), alpha = self.bnalpha, name = 'a_mu')
        self.l_a_var = batch_norm(DenseLayer(self.l_x_to_a, self.dima, W = W_init,b = b_init, nonlinearity=None,
                                             name = 'a_var'), alpha = self.bnalpha, name = 'a_var')
        ...
        self.l_z_mu = batch_norm(DenseLayer(self.l_axy, self.dimz,W=W_init, b=b_init, nonlinearity=None,
                                            name='z_mu'), alpha = self.bnalpha, name = 'z_mu')
        self.l_z_var = batch_norm(DenseLayer(self.l_axy, self.dimz,W=W_init, b=b_init, nonlinearity=None,
                                             name='z_var'), alpha = self.bnalpha, name = 'z_var')
        ...
        self.l_u_mu = batch_norm(DenseLayer(self.l_yz, self.dima,W=W_init, b=b_init, nonlinearity=None,
                                            name='u_mu'), alpha = self.bnalpha, name = 'u_mu')
        self.l_u_var = batch_norm(DenseLayer(self.l_yz, self.dima,W=W_init, b=b_init, nonlinearity=None,
                                             name='u_var'), alpha = self.bnalpha, name = 'u_var')
        ...
        self.l_hid = batch_norm( DenseLayer(self.l_uyz, num_units= self.dec_num_units, W = W_init_act, b= b_init,
