...
class DeepModel():
    ...
    def build_model(self, use_mean_lstm=False, act_fun=lasagne.nonlinearities.tanh,  # density layer active function
        ...
        if theano.config.floatX == 'float32':
            self.b = theano.shared(np.float32(8.0))
        else:
            self.b = theano.shared(np.float64(8.0))
    ...
    def _get_baselines(self, incomings):
        ...
        return self.b * T.sum(mask, axis=1)
    ...
    def train_sample_function(self):
        ...
        total_cost_directly = -T.sum(loss_entropy) + T.sum(loss_rec + sym_klw * loss_kl)
        ...
        all_grads = total_norm_constraint(all_grads, max_norm=self.max_norm)
        updates = adam(all_grads, all_params, self.lr)
        #update_baseline = {self.b: 0.99 * self.b + 0.01 * T.sum(loss_rec_u)/T.sum(sym_mask_u)}
