...
def init_config( ):
    ...
    params['annealing_center'] = 10.0
    params['annealing_width'] = 1.0 #*10
    params['keep_rate'] = 0.75 #>1 keep all <0 inputless model
    params['drop_out'] = -1.0
    params['dev_per'] = 1
    params['save_per'] = 10
...
batchitertrain = BatchIterator(range(params['num_train']), params['batch_size'], data=(s_train, mask_train))
batchitertrain = threaded_generator(batchitertrain,3)

batchitertest = BatchIterator(range(params['num_dev']), params['batch_size'], data=(s_dev, mask_dev))
...
cost = l_vae.get_cost(params['keep_rate'], params['drop_out'], s_clip, sym_mask, sym_w)
...
if params['keep_rate'] > 0:# <=0 means inputless
    eval_result = l_vae.get_cost(2.0, -1.0, sym_s , sym_mask , sym_w)
else:
    eval_result = l_vae.get_cost(-2.0, -1.0, sym_s , sym_mask , sym_w)
